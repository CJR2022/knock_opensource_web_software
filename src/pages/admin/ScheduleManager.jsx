...
export default function ScheduleManager() {
    ...
    const handleAddSchedule = async () => {
        if (!newStartTime || !newEndTime || !selectedAdminId) {
            alert("시작 시간, 종료 시간, 그리고 담당 관리자를 모두 선택해주세요.");
            return;
        }
    ...
    const handleDeleteSchedule = async (scheduleId) => {
        if (!window.confirm("이 근무 일정을 삭제하시겠습니까?")) return;
        try {
    ...
    const handleEditSave = async (scheduleId) => {
        if (!editAdminId) return alert("변경할 관리자를 선택해주세요.");
        try {
    ...
    const handleAddClosedDay = async () => {
        if (!closeReason.trim()) return alert("휴무 사유를 입력해주세요.");
        try {
    ...
    const handleDeleteClosedDay = async (dayId) => {
        if (!window.confirm("휴무일 지정을 해제하시겠습니까?")) return;
        try {
            ...
            <div className="mt-4">
                <div className="grid grid-cols-7 gap-1 text-center border-b border-gray-100 pb-3 mb-3">
                    {korWeekDays.map((d, idx) => (
                        <div key={d} className={`text-[13px] font-extrabold ${idx === 0 ? 'text-red-400' : idx === 6 ? 'text-blue-400' : 'text-gray-400'}`}>
                            {d}
                        </div>
                    ))}
                </div>
                ...
                <div className="card p-6 flex-1 md:max-w-md w-full">
                    <div className="schedule-header" style={{ borderBottom: 'none', paddingBottom: 0 }}>
                        ...
                        <div className="flex gap-2">
                            <button onClick={prevMonth} className="week-btn text-gray-600 font-bold px-3 hover:text-black">
                                &lt;
                            </button>
                            <button onClick={nextMonth} className="week-btn text-gray-600 font-bold px-3 hover:text-black">
                                &gt;
                            </button>
                        </div>
                    </div>
                        ...
                        <>
                            <div className="card p-6">
                                <div className="schedule-header">
                                    <h3 className="schedule-section-title">
                                        {selectedDate.getMonth() + 1}월 {selectedDate.getDate()}일 ({korWeekDays[selectedDate.getDay()]}) 근무 현황
                                    </h3>
                                </div>
                                                ...
                                                <div key={schedule.id} className="schedule-card-item">
                                                    <div className="flex flex-col gap-1 w-full">
                                                        <div className="flex items-center">
                                                            <span className="time-btn2 active px-4 py-2 pointer-events-none">
                                                        ...
                                                        {isEditing ? (
                                                            ...
                                                            <span className="schedule-admin-label">
                                                                담당 관리자: <span className="text-black">{schedule.admin_name}</span>
                                                            </span>
                                                        )}
                                                    </div>
                                                    ...
                                                    <div className="schedule-action-group">
                                                        {isEditing ? (
                                                            <>
                                                                <button onClick={() => handleEditSave(schedule.id)} className="schedule-action-btn blue">저장</button>
                                                                <button onClick={handleEditCancel} className="schedule-action-btn text-gray-500">취소</button>
                                                            </>
                                                        ) : (
                                                            <>
                                                                <button onClick={() => handleEditStart(schedule)} className="schedule-action-btn blue">수정</button>
                                                                <button onClick={() => handleDeleteSchedule(schedule.id)} className="schedule-action-btn red">삭제</button>
                                                            </>
                                                        )}
                                                    </div>
                                                </div>
                            ...
                            <div className="flex flex-col lg:flex-row gap-6">
                                    ...
                                    <div className="flex flex-col gap-4">
                                        <div className="flex gap-2">
                                            <div className="flex-1">
                                                <select value={newStartTime} onChange={(e) => setNewStartTime(e.target.value)} className="schedule-form-select">
                                                    <option value="" disabled>시작</option>
                                                    {timeOptions.map(time => <option key={`start-${time}`} value={time}>{time}</option>)}
                                                </select>
                                            </div>
                                            <div className="flex-1">
                                                <select value={newEndTime} onChange={(e) => setNewEndTime(e.target.value)} className="schedule-form-select">
                                        ...
                                        <div>
                                            <select value={selectedAdminId} onChange={(e) => setSelectedAdminId(e.target.value)} className="schedule-form-select">
                                                <option value="" disabled>담당 관리자</option>
                                                {adminList.map(admin => <option key={admin.id} value={admin.id}>{admin.name}</option>)}
                                            </select>
                                        </div>
                                    </div>
                                    <div className="mt-6">
                                ...
                                <div className="card p-6 flex-1 border-t-[3px] border-t-red-500">
                                    ...
                                    <p className="text-xs text-gray-500 font-bold mb-4">* 선택하신 {selectedDate.getDate()}일 휴무일로 지정합니다.</p>
