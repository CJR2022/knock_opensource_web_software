...
import StudentManageModal from "./modal/StudentManageModal";
        ...
        <div className="page">
            <div className="flex flex-col gap-6">
                <label className="searchBar p-4">
                    ...
                    <input type="text" className="search flex-1" placeholder="학번 혹은 이름을 입력하여 검색"
                           onChange={(e) => setInput(e.target.value)}></input>
                </label>

                <div className="tabBar">
                    <button
                        className={tab === "active" ? "userTab active" : "userTab"}
                        onClick={() => setTab("active")}
                    >
                        기존 가입자 ({activeStudents.length})
                    </button>
                    <button
                        className={tab === "pending" ? "userTab active" : "userTab"}
                        onClick={() => setTab("pending")}
                    >
                        신규 가입자 ({pendingStudents.length})
                    </button>
                </div>
                                        ...
                                        <div key={student.id} className="card newStudent p-5">
                                            ...
                                            <div className="card-footer">
                                                ...
                                                <button className="approve-btn"
                                                        onClick={() => handleApprove(student.id)}>승인 하기
                                                </button>
                                            </div>
                                        </div>);
                                    return;
                    ...
                    <section>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {activeStudents.map((student) => {
                                    if (student.name.includes(input) || String(student.student_number).includes(input)) {
                                        return (
                                            <div key={student.id} className="card active student p-5" onClick={() => {
                                                ...
                                                <div className="rental-info">
                                                    <p>대여
                                                        물품: <strong>{student.current_rentals.length > 0 ? student.current_rentals.map(r => r.item_name).join(', ') : '없음'}</strong>
                                                    </p>
                ...
                {manageModalOpen && selectedStudent && (
                    <StudentManageModal
